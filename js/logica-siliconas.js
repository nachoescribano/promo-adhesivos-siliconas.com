...
function centrarInfografias(){
	var anchuraPantalla=parseInt(window.innerWidth);
	var inforgrafiaContainer=document.getElementsByClassName("infografias");

	if(anchuraPantalla<800){
		for(var i=0;i<inforgrafiaContainer.length;i++){
			var buffer=inforgrafiaContainer[i].innerHTML;
			inforgrafiaContainer[i].innerHTML="<center>"+buffer+"</center>";
		}
	}	
}

function descentrarLasInfografias(){
	var inforgrafiaContainer=document.getElementsByClassName("infografias");
		for(var i=0;i<inforgrafiaContainer.length;i++){
			var buffer=inforgrafiaContainer[i].innerHTML;

			buffer=buffer.replace("<center>",'');
			buffer=buffer.replace("</center>",'');
			inforgrafiaContainer[i].innerHTML=buffer;
		}
}
